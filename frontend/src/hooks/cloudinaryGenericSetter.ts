...
export const useCloudinaryGenericSetter = (
  uploadPreset: string,
  tag: string,
  ...
  const [secureUrl, setUploadedFilename] = useState("");
  const [publicId, setUploadedFilenamePublic] = useState("");
  const clearData = () => {
    setUploadedFilename("");
...
export const useCloudinaryVariableSetter = (
  uploadPreset: string,
  tag: string,
  signFunction: Function,
  uploadSignPresetUrl: string
  ...
  Dispatch<SetStateAction<string>>,
  Dispatch<SetStateAction<string>>,
  (setFieldValue: any, name: any) => void,
  () => void
] => {
  // alert(JSON.stringify(defaultValue))
  const [secureUrl, setUploadedFilename] = useState("");
  ...
  const upload = async (setFieldValue: any, name: any) => {
    const res = await signFunction(uploadSignPresetUrl);
    // // console.log(res);
    const myWidget = window.cloudinary.openUploadWidget(
      {
