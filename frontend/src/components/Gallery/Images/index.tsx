...
const Images = () => {
  ...
  //   // console.log(selectedItem);
  //   // setPageCurrent(selectedItem.selected);
  //   dispatch(fetchImagesThunk({ page: selectedItem.selected + 1, limit: Number.MAX_SAFE_INTEGER }));
  // };
  ...
  const CustomRightArrow = ({ onClick, ...rest }: any) => {
    ...
    return (<button
      onClick={onClick}
      className="react-multiple-carousel__arrow react-multiple-carousel__arrow--right" style={{"right": "2px", borderRadius: "0.375rem", backgroundColor: "rgba(0, 0, 0, 0.35)"}}>
  ...
  const CustomLeftArrow = ({ onClick, ...rest }: any) => {
    ...
    return (<button
      onClick={onClick}
      className="react-multiple-carousel__arrow react-multiple-carousel__arrow--left my-auto" style={{"left": "3px", borderRadius: "0.375rem", backgroundColor: "rgba(0, 0, 0, 0.35)"}}>
      ...
      <div className="mt-10">
        ...
        <div className="mx-auto w-[90%] ">
          {loadingImages == "pending" ? <p>Loading</p> : (<div className="h-full">
            <RMCarousel responsive={responsiveCarousel} infinite={true} customRightArrow={<CustomRightArrow />} customLeftArrow={<CustomLeftArrow/>}>
              {galleryImages.map((galleryImage, indx) => {
                return (
                  <div className="card text-left m-1 h-full py-5" key={indx} onClick={() => onClickHandler(galleryImage.imageUrl)} data-bs-toggle="modal"
                  ...
                  role="presentation">
                    <img
                      ...
                      alt="Alt text"
                    />
                    {galleryImage.description.length > 0 ? <div className="card-body absolute">
              ...
              <div className="modal-dialog modal-dialog-centered pointer-events-none relative w-full h-full">
                <div className="modal-content pointer-events-auto  relative flex w-full h-full flex-col rounded-md  border-none bg-clip-padding text-current outline-none">
                  <div className="modal-header flex flex-shrink-0 items-center justify-between rounded-t-md p-4">
                    <button
                      type="button"
                      className="btn-close absolute right-0 z-[200] mx-3 mt-10 box-content rounded-none border-none p-1 text-white opacity-50 hover:text-black hover:no-underline hover:opacity-75 focus:opacity-100 focus:shadow-none focus:outline-none"
                      onClick={() => {
                        setShowModal(false);
                        toggleScrollbar();
                      }}
