...
import { toggleReadMoreRegisterModal } from "../../../store/membershipForm/membershipFormSlice";
...
export function RegistrationImage({ value, column, row }: any) {
  if (value)
    return (
      <span className="flex w-full flex-row items-center gap-4">
        <span className="h-32  w-52 ">
          <img
            src={value.secureUrl}
            className="mr-5 h-32  w-52 rounded-md object-cover"
            alt={row.original[column.flagAccessor].publicId}
          />
        </span>
        <span
          className={classNames(
          ...
          <a href={value.secureUrl}>
            {/* {value.split("ingredient/")[1] ||
              value
                ...
                .slice(0, 20)} */}
            {row.original[column.flagAccessor].publicId.slice(0, 30)}
          </a>
        </span>
      </span>
    );
...
export function RegistrationText({ value, column, row }: any) {
  if (value)
    return (
      <span className="flex w-full flex-row items-center gap-4">
        {value}
...
export function ReadMoreRegister({ value, column, row }: any) {
  const dispatch = useAppDispatch();
  return (
    <>
      {!value? <span>-</span>:<>
          ...
          <>
            ...
            >
              <button
                ...
                onClick={() => {
                  // // console.log("Edit Modal", row.original[column.editAccessor]);
                  dispatch(toggleReadMoreRegisterModal(value));
                }}
                data-mdb-ripple="true"
                data-bs-toggle="modal"
                data-bs-target={`#ReadMoreModalView`}
