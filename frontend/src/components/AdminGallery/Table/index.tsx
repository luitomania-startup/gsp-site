...
export function ImageCell({ value, column, row }: any) {
  if (value)
    return (
      <span className="flex w-full flex-row items-center gap-4">
        <span className="w-52">
          <img
            alt=""
            src={value}
            className="min-w-52 mr-5 h-32 rounded-md object-cover"
          />
        </span>
        <span
          className={classNames(
...
export function VideoCell({ value, column, row }: any) {
  ...
  const imgLink = value.slice(0, value.length - extLength - 1) + ".jpg";
  if (value)
    return (
      <span className="flex w-full flex-row items-center gap-4">
        <span className="w-52">
          ...
          <>
            ...
            >
              <button
                ...
                onClick={() => {
                  // // console.log("Edit Modal", row.original[column.editAccessor]);
                  dispatch(toggleReadMoreGalleryModal(value));
                }}
                data-mdb-ripple="true"
                data-bs-toggle="modal"
