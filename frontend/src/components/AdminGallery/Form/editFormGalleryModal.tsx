...
import ImageEditForm from "./ImageForm/editForm";
import VideoEditForm from "./VideoForm/editForm";
...
const ImagesAddModal = ({galleryMode}: {galleryMode: string}) => {
            ...
            >
              <div className="flex items-center justify-between rounded-t border-b p-4 dark:border-gray-600">
                <h3 className="text-xl font-semibold text-gray-900 dark:text-white ">
                  {`Edit ${galleryMode === "Images" ? "an Image" : "a Video"}`}
                </h3>
                <button
                  type="button"
                  ...
                  onClick={() => dispatch(toggleEditGalleryModal())}
                >
                  <img
                    src={CloseIcon}
                    alt="close"
              ...
              {galleryMode === "Images" ? <ImageEditForm /> : <VideoEditForm />}
            </div>
          </div>
        </div>
      </div>
