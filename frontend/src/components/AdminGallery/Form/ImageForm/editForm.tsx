...
const EditFormModal = () => {
  ...
  const initialImage = useAppSelector(selectImageOne);
  useEffect(() => {
    dispatch(fetchImageThunk(id));
  }, []);
  // useEffect(() => {
  //   setTimeout(() => {
  //     if(initialImage._id === ""){
  ...
  useEffect(() => {
    setUploadedFilename(initialImage?.imageUrl || "");
    setUploadedFilenamePublic(initialImage.imagePublicId || "");
  }, [initialImage]);
  // console.log("URL", secureUrl, publicId)
  // const formInitialValues = {
  //   _id: initialImage._id,
  //   description: initialImage.description,
  //   imagePublicId: initialImage.imagePublicId,
      ...
      {!["idle", "pending"].includes(loadingState) ? (
        <Formik
          ...
          onSubmit={(values, { setSubmitting, resetForm }) => {
            ...
            dispatch(editImageThunk({ formData, page: 1, limit: 4 })).unwrap()
            .then(() => {
            ...
            <div className=" my-10 flex px-4 ">
              ...
              /> */}
                  <Editor
                    ...
                    initialValue={initialImage.description}
                    init={{
                      content_style:
                ...
                <div className="form-group row py-sm-2  px-sm-3 my-2 pb-5">
                  ...
                  >
                    {secureUrl !== "" ? (
                      <>
                        <span className="text-black">Image uploaded at</span>{" "}
                        <span>{secureUrl}</span>{" "}
                      </>
                    ) : (
                      ""
