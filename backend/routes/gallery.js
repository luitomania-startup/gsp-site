...
router.get("/image", async (req, res) => {
  ...
  const count = await Image.countDocuments({ isDeleted: false });
...
router.get("/video", async (req, res) => {
  ...
  const page = parseInt(req.query.page) || 1;
  const limit = parseInt(req.query.limit) || 4;
  const count = await Video.countDocuments({ isDeleted: false });
...
router.post("/image", async (req, res) => {
  ...
  const { error } = validateImage(sanitizeObject(req.body));
...
router.post("/video", async (req, res) => {
  ...
  const { error } = validateVideo(sanitizeObject(req.body));
  if (error) return res.status(400).send(error.details[0].message);
